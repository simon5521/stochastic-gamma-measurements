...
class SampleImageStream():
	...
	def raiseNewData(self,blurred, car):
		...
		#hu.bme.mit.gamma.expression.model.impl.DecimalTypeDefinitionImpl@5ca4c333
		self.event_cntr=self.event_cntr+1
	...
	def __init__(self,detmodel):
		...
		self.components.update({ "System()Camera1().softwareTimer" :
			PeriodicEventSource(
				...
				rules = {"Events" : {
					#[]
					#[hu.bme.mit.gamma.environment.model.impl.StochasticRuleImpl@6ca7a65b]
					"NewEvent" : 
					ContinuousRandomVariable(pyro.distributions.Normal(loc=torch.tensor(0.2),scale=torch.tensor(0.02)),"ContRandomVarriablesoftwareTimer56")
				}}
				,
				portevents = {
				"Events" : [
		...
		self.components.update({ "System()Camera2().softwareTimer" :
			PeriodicEventSource(
				...
				rules = {"Events" : {
					...
					#[hu.bme.mit.gamma.environment.model.impl.StochasticRuleImpl@6ca7a65b]
					"NewEvent" : 
					ContinuousRandomVariable(pyro.distributions.Normal(loc=torch.tensor(0.2),scale=torch.tensor(0.02)),"ContRandomVarriablesoftwareTimer57")
				}}
				,
				portevents = {
				"Events" : [
		...
		self.components.update({ "System()Traffic().carArrival" :
			PeriodicEventSource(
				...
				,
				rules = {"Cars" : {
					#[hu.bme.mit.gamma.environment.model.impl.StochasticRuleImpl@64f1d9d8]
					#
					"NewEvent" : 
					ContinuousRandomVariable(pyro.distributions.Exponential(torch.tensor(3.0)),"ContRandomVarriablecarArrival58")
				}}
				,
				portevents = {
				"Cars" : [
		...
		self.components.update({ "System()Traffic().carDelay" :
			DelayEventStream(
				...
				,
				rules = {"CarOut" : {
					#[hu.bme.mit.gamma.environment.model.impl.StochasticRuleImpl@3dc40f8e]
					#
					"NewEvent" : 
					ContinuousRandomVariable(pyro.distributions.Normal(loc=torch.tensor(0.5),scale=torch.tensor(0.1)),"ContRandomVarriablecarDelay59")
				}}
				,
				simulator=self)})
		...
		self.components.update({ "System()Camera1()Software()CameraSoftware().imageBlur" :
			SampleImageStream(
				...
				,
				rules = {"ImageOut" : {
					#[hu.bme.mit.gamma.environment.model.impl.StochasticRuleImpl@79938160]
					#
					"NewData" : 
					ContinuousRandomVariable(pyro.distributions.Bernoulli(torch.tensor(0.1)),"DiscRandomVarriableimageBlur62")
				}}
				,
				simulator=self)})
		...
		self.components.update({ "System()Camera2()Software()CameraSoftware().imageBlur" :
			SampleImageStream(
				...
				rules = {"ImageOut" : {
					...
					#
					"NewData" : 
					ContinuousRandomVariable(pyro.distributions.Bernoulli(torch.tensor(0.1)),"DiscRandomVarriableimageBlur63")
				}}
				,
				simulator=self)})
