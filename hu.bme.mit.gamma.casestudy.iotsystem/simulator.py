...
class SampleImageStream():
	...
	def raiseNewData(self,blurred, car):
		...
		#hu.bme.mit.gamma.expression.model.impl.DecimalTypeDefinitionImpl@1b8bc78c
		self.event_cntr=self.event_cntr+1
	...
	def __init__(self,detmodel):
		...
		self.components.update({ "System()Camera1().softwareTimer" :
			PeriodicEventSource(
				...
				rules = {"Events" : {
					#[]
					#[hu.bme.mit.gamma.environment.model.impl.StochasticRuleImpl@114fe054]
					"NewEvent" : 
					ContinuousRandomVariable(pyro.distributions.Normal(loc=torch.tensor(0.2),scale=torch.tensor(0.02)),"ContRandomVarriablesoftwareTimer24")
				}}
				,
				portevents = {
				"Events" : [
		...
		self.components.update({ "System()Camera2().softwareTimer" :
			PeriodicEventSource(
				...
				rules = {"Events" : {
					...
					#[hu.bme.mit.gamma.environment.model.impl.StochasticRuleImpl@114fe054]
					"NewEvent" : 
					ContinuousRandomVariable(pyro.distributions.Normal(loc=torch.tensor(0.2),scale=torch.tensor(0.02)),"ContRandomVarriablesoftwareTimer25")
				}}
				,
				portevents = {
				"Events" : [
		...
		self.components.update({ "System()Traffic().carArrival" :
			PeriodicEventSource(
				...
				,
				rules = {"Cars" : {
					#[hu.bme.mit.gamma.environment.model.impl.StochasticRuleImpl@45d241c7]
					#
					"NewEvent" : 
					ContinuousRandomVariable(pyro.distributions.Exponential(torch.tensor(3.0)),"ContRandomVarriablecarArrival26")
				}}
				,
				portevents = {
				"Cars" : [
		...
		self.components.update({ "System()Traffic().carDelay" :
			DelayEventStream(
				...
				,
				rules = {"CarOut" : {
					#[hu.bme.mit.gamma.environment.model.impl.StochasticRuleImpl@75892e01]
					#
					"NewEvent" : 
					ContinuousRandomVariable(pyro.distributions.Normal(loc=torch.tensor(0.5),scale=torch.tensor(0.1)),"ContRandomVarriablecarDelay27")
				}}
				,
				simulator=self)})
		...
		self.components.update({ "System()Camera1()Software()CameraSoftware().imageBlur" :
			SampleImageStream(
				...
				,
				rules = {"ImageOut" : {
					#[hu.bme.mit.gamma.environment.model.impl.StochasticRuleImpl@5f057b]
					#
					"NewData" : 
					ContinuousRandomVariable(pyro.distributions.Bernoulli(torch.tensor(0.1)),"DiscRandomVarriableimageBlur30")
				}}
				,
				simulator=self)})
		...
		self.components.update({ "System()Camera2()Software()CameraSoftware().imageBlur" :
			SampleImageStream(
				...
				rules = {"ImageOut" : {
					...
					#
					"NewData" : 
					ContinuousRandomVariable(pyro.distributions.Bernoulli(torch.tensor(0.1)),"DiscRandomVarriableimageBlur31")
				}}
				,
				simulator=self)})
