...
class SampleImageStream():
	...
	def raiseNewData(self,blurred, car):
		...
		#hu.bme.mit.gamma.expression.model.impl.DecimalTypeDefinitionImpl@1e3fdcc9
		self.event_cntr=self.event_cntr+1
...
class StochasticEventGenerator():
	...
	def __init__(self,detmodel):
		...
		self.delayMean=torch.tensor([0.0])
		#0
		#0
		...
		self.components.update({ "System()Camera1().softwareTimer" :
			PeriodicEventSource(
				...
				rules = {"Events" : {
					#[]
					#[hu.bme.mit.gamma.environment.model.impl.StochasticRuleImpl@68651d74]
					"NewEvent" : 
					ContinuousRandomVariable(pyro.distributions.Normal(loc=torch.tensor(0.2),scale=torch.tensor(0.02)),"ContRandomVarriablesoftwareTimer40")
				}}
				,
				portevents = {
				"Events" : [
		...
		self.components.update({ "System()Camera2().softwareTimer" :
			PeriodicEventSource(
				...
				rules = {"Events" : {
					...
					#[hu.bme.mit.gamma.environment.model.impl.StochasticRuleImpl@68651d74]
					"NewEvent" : 
					ContinuousRandomVariable(pyro.distributions.Normal(loc=torch.tensor(0.2),scale=torch.tensor(0.02)),"ContRandomVarriablesoftwareTimer41")
				}}
				,
				portevents = {
				"Events" : [
		...
		self.components.update({ "System()Traffic().carArrival" :
			PeriodicEventSource(
				...
				,
				rules = {"Cars" : {
					#[hu.bme.mit.gamma.environment.model.impl.StochasticRuleImpl@26fd51f0]
					#
					"NewEvent" : 
					ContinuousRandomVariable(pyro.distributions.Exponential(torch.tensor(2.5)),"ContRandomVarriablecarArrival42")
				}}
				,
				portevents = {
				"Cars" : [
		...
		self.components.update({ "System()Traffic().carDelay" :
			DelayEventStream(
				...
				,
				rules = {"CarOut" : {
					#[hu.bme.mit.gamma.environment.model.impl.StochasticRuleImpl@1816319a]
					#
					"NewEvent" : 
					ContinuousRandomVariable(pyro.distributions.Normal(loc=self.delayMean[0],scale=torch.tensor(0.1)),"ContRandomVarriablecarDelay43")
				}}
				,
				simulator=self)})
		...
		self.components.update({ "System()Camera1()Software()CameraSoftware().imageBlur" :
			SampleImageStream(
				...
				,
				rules = {"ImageOut" : {
					#[hu.bme.mit.gamma.environment.model.impl.StochasticRuleImpl@62bfb3ec]
					#
					"NewData" : 
					ContinuousRandomVariable(pyro.distributions.Bernoulli(torch.tensor(0.1)),"DiscRandomVarriableimageBlur46")
				}}
				,
				simulator=self)})
		...
		self.components.update({ "System()Camera2()Software()CameraSoftware().imageBlur" :
			SampleImageStream(
				...
				rules = {"ImageOut" : {
					...
					#
					"NewData" : 
					ContinuousRandomVariable(pyro.distributions.Bernoulli(torch.tensor(0.1)),"DiscRandomVarriableimageBlur47")
				}}
				,
				simulator=self)})
	...
	def reset(self):
		...
		for dist in self.dists:
			dist.reset()
		self.delayMean[0]=pyro.sample("param_0",pyro.distributions.Uniform(low=torch.tensor(0.1),high=torch.tensor(1.0))
		)
		#0
