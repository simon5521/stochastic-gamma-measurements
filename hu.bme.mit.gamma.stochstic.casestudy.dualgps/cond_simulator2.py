...
startJVM(getDefaultJVMPath(), '-ea',
         '-Djava.class.path=' + str(os.path.realpath(__file__).replace(os.path.basename(__file__),"")) + '/bin'
         )
...
class StochasticEventGenerator():
	...
	def __init__(self,detmodel):
		...
		self.voterFailureRate=torch.tensor([0.0])
		#0
		#0
		...
		self.components.update({ "System().GPS1_Failure" :
			EventSource(
				...
				,
				rules = {"Faults" : {
					#[hu.bme.mit.gamma.environment.model.impl.StochasticRuleImpl@20c1fe97]
					#
					"Failure" : 
					ContinuousRandomVariable(pyro.distributions.Exponential(torch.tensor(10.0)),"ContRandomVarriableGPS1_Failure15")
				}}
				,
				portevents = {
				"Faults" : [
		...
		self.components.update({ "System().GPS2_Failure" :
			EventSource(
				...
				,
				rules = {"Faults" : {
					#[hu.bme.mit.gamma.environment.model.impl.StochasticRuleImpl@2cd37eac]
					#
					"Failure" : 
					ContinuousRandomVariable(pyro.distributions.Exponential(torch.tensor(10.0)),"ContRandomVarriableGPS2_Failure16")
				}}
				,
				portevents = {
				"Faults" : [
		...
		self.components.update({ "System().Voter_Failure" :
			EventSource(
				...
				,
				rules = {"Faults" : {
					#[hu.bme.mit.gamma.environment.model.impl.StochasticRuleImpl@31acf532]
					#
					"Failure" : 
					ContinuousRandomVariable(pyro.distributions.Exponential(self.voterFailureRate[0]),"ContRandomVarriableVoter_Failure17")
				}}
				,
				portevents = {
				"Faults" : [
	...
	def reset(self):
		...
		for dist in self.dists:
			dist.reset()
		self.voterFailureRate[0]=pyro.sample("param_0",pyro.distributions.Uniform(low=torch.tensor(0.0),high=torch.tensor(20.0))
		)
		#0
		self.detmodel.reset(self.voterFailureRate)
