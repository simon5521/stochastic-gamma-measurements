    ...
    def reset(self):
        ...
        self.components.update({ "System().GPS1_Failure" :
            EventSource(
                ...
                rules = {
                "Faults" : {
                #[hu.bme.mit.gamma.environment.model.impl.StochasticRuleImpl@4091ccc2]
                #
                "Failure" : 
                ContinuousRandomVariable(pyro.distributions.Exponential(torch.tensor(10.0
                )),"ContRandomVarriableGPS1_Failure9")
                }
                }
                ,
                portevents = {
        ...
        self.components.update({ "System().GPS2_Failure" :
            EventSource(
                ...
                rules = {
                "Faults" : {
                #[hu.bme.mit.gamma.environment.model.impl.StochasticRuleImpl@103236a2]
                #
                "Failure" : 
                ContinuousRandomVariable(pyro.distributions.Exponential(torch.tensor(10.0
                )),"ContRandomVarriableGPS2_Failure10")
                }
                }
                ,
                portevents = {
        ...
        self.components.update({ "System().Voter_Failure" :
            EventSource(
                ...
                rules = {
                "Faults" : {
                #[hu.bme.mit.gamma.environment.model.impl.StochasticRuleImpl@1c582875]
                #
                "Failure" : 
                ContinuousRandomVariable(pyro.distributions.Exponential(self.voterFailureRate
                ),"ContRandomVarriableVoter_Failure11")
                }
                }
                ,
                portevents = {
...
def simulate():
    ...
    stochmodel.voterFailureRate=pyro.sample("param_0",pyro.distributions.Uniform(low=torch.tensor(0.0),high=torch.tensor(10.0)))
    stochmodel.reset()
    ...
    stochmodel.time
    pyro.deterministic("SystemCommunicationFailstop",torch.tensor(stochmodel.time))
