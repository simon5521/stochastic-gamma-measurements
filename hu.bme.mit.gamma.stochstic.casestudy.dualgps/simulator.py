	...
	def __init__(self,detmodel):
		...
		self.components.update({ "System().GPS1_Failure" :
			EventSource(
				...
				,
				rules = {"Faults" : {
					#[hu.bme.mit.gamma.environment.model.impl.StochasticRuleImpl@520cbf1e]
					#
					"Failure" : 
					ContinuousRandomVariable(pyro.distributions.Exponential(torch.tensor(10.0)),"ContRandomVarriableGPS1_Failure3")
				}}
				,
				portevents = {
				"Faults" : [
		...
		self.components.update({ "System().GPS2_Failure" :
			EventSource(
				...
				,
				rules = {"Faults" : {
					#[hu.bme.mit.gamma.environment.model.impl.StochasticRuleImpl@2dcb7d8f]
					#
					"Failure" : 
					ContinuousRandomVariable(pyro.distributions.Exponential(torch.tensor(10.0)),"ContRandomVarriableGPS2_Failure4")
				}}
				,
				portevents = {
				"Faults" : [
		...
		self.components.update({ "System().Voter_Failure" :
			EventSource(
				...
				,
				rules = {"Faults" : {
					#[hu.bme.mit.gamma.environment.model.impl.StochasticRuleImpl@31698978]
					#
					"Failure" : 
					ContinuousRandomVariable(pyro.distributions.Exponential(torch.tensor(20.0)),"ContRandomVarriableVoter_Failure5")
				}}
				,
				portevents = {
				"Faults" : [
